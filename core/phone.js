...
const Phone = (() => {
    ...
    async function init() {
        ...
        if ('serviceWorker' in navigator) {
            ...
            catch (e) { Logger.warn('SW registration failed', e.message); }
        }
    ...
    function createAppIcon(appInfo) {
        ...
        wrapper.addEventListener('pointerdown', () => {
            pressTimer = setTimeout(() => showAppContextMenu(event, appInfo), 500);
        });
    ...
    function showAppContextMenu(e, appInfo) {
        ...
        const y = e.clientY - phoneRect.top + 8;
    ...
    function setupSwipe() {
        ...
        vp.addEventListener('touchend', onSwipeEnd);vp.addEventListener('mousedown', (e) => { isMouseDown = true; onSwipeStart({ touches: [e] }); });
        vp.addEventListener('mousemove', (e) => { if (!isMouseDown) return; onSwipeMove({ touches: [e], preventDefault: () => e.preventDefault() }); });
    ...
    function onSwipeMove(e) {
        ...
        if (isDragging) {
            ...
            touchDeltaX = dx;
            const offset = -(currentPage * 100) + (dx / document.getElementById('desktop-viewport').offsetWidth) * 100;
    ...
    function showContextMenu(x, y, items) {
        ...
        const itemsHtml = items.map((item, i) => {
            ...
            return `<div class="context-menu-item ${item.danger ? 'danger' : ''}" data-action="${i}">
                <span>${item.label}</span><span style="opacity:0.4">${item.icon || ''}</span>
            </div>`;
        }).join('');
    ...
    async function refreshDesktop() { await renderDesktop(); }

    return { init, showModal, closeModal, showContextMenu, closeContextMenu, refreshDesktop, loadSettings };
})();
